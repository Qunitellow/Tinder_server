...
public interface UsersRepository extends CrudRepository<User, Long> {
    ...
    @Query(nativeQuery = true, value = "select count(*) from users where id != ?1")
    int getNumberOfUsersAnyGender(Long id);

    Optional<User> findByUsername(String username);

    @Query(nativeQuery = true, value = "update users set description = ?2 where id = ?1")
    void editingDescription(Long id, String Desc);
